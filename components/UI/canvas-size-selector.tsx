...
export const CanvasSizeSelector: React.FC<CanvasSizeSelectorProps> = ({ className }) => {
  const [isOpen, setIsOpen] = useState(false);
  const { width, height, setCanvasSize } = useCanvasStore();

  const currentSize = CANVAS_SIZES.find(size => size.width === width && size.height === height);
  const currentSizeLabel = currentSize ? currentSize.name : `${width} × ${height}`;
      ...
      {isOpen && (
        <>
          ...
          <div className="absolute top-full mt-1 w-64 bg-white border border-gray-300 rounded-md shadow-lg z-20 max-h-96 overflow-y-auto">
                ...
                {sizes.map((size) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 hover:bg-gray-50 focus:outline-none focus:bg-gray-50 ${
                      currentSize?.name === size.name ? 'bg-blue-50 text-blue-700' : 'text-gray-700'
                    }`}
                  >
            ...
            <button
              onClick={() => {
                const customWidth = prompt('Enter width (px):', width.toString());
                const customHeight = prompt('Enter height (px):', height.toString());
